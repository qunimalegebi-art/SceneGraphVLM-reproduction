...
```text
老师/师兄您好，我现在需要在远程 GPU 上复现 SceneGraphVLM 的 baseline。
想确认一下服务器使用方式：

1. 服务器 IP、用户名、登录方式是什么？是否需要 VPN？
2. GPU 型号和显存是多少？我可以使用几张卡？
...
我计划先跑 1 张图 smoke test，再跑小规模 PVSG / 真实视频 baseline，不会一开始直接跑全量任务。
```
