...
export COMET_PROJECT_NAME="${COMET_PROJECT_NAME:-qwen3_5_1b_grpo_pvsg}"
...
swift rlhf \
  --rlhf_type grpo \
  --model "${MODEL_ROOT}/work_dirs/baseannot-wout-zerorel" \
  --model_type qwen3_5 \
  --template qwen3_5 \
  ...
  --dataset "${DATA_ROOT}/PVSG_json/pvsg_base_annot_gt_prompt/grpo/train_noprevgraph.jsonl" \
  --val_dataset "${DATA_ROOT}/PVSG_json/pvsg_base_annot_gt_prompt/grpo/eval_noprevgraph.jsonl" \
  ...
  --external_plugins "${GRPO_ROOT}/swift/rewards/pvsg_psg_toon_reward.py" \
  --reward_funcs \
  ...
  --save_steps 1000 \
  --save_total_limit 30 \
  --output_dir "${MODEL_ROOT}/qwen3_5_1b_grpo_pvsg_noprevgraph_plusprec" \
