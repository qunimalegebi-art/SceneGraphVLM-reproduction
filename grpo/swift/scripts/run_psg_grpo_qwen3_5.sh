...
export COMET_PROJECT_NAME="${COMET_PROJECT_NAME:-qwen3_5_1b_grpo_psg}"
...
swift rlhf \
  --rlhf_type grpo \
  --model "${MODEL_ROOT}/work_dirs/PSG/checkpoint-11424" \
  --model_type qwen3_5 \
  --template qwen3_5 \
  ...
  --dataset "${DATA_ROOT}/PSG_json/grpo/train.jsonl" \
  --val_dataset "${DATA_ROOT}/PSG_json/grpo/eval.jsonl" \
  ...
  --save_steps 1000 \
  --save_total_limit 30 \
  --output_dir "${MODEL_ROOT}/qwen3_5_1b_grpo_psg_noprevgraph" \
