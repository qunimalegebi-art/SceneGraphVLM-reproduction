...
# ============================================================
# Action Genome reward ORMs for Swift, adapted from PVSG reward
# logic with minimal changes.
# - External format: AG TOON with rel_pairs[...] rows
# - Internal edge representation: atomic triplets derived from
#   attention / spatial / contacting label lists.
# ============================================================
...
AG_ATTENTION_MAX = int(os.getenv("AG_ATTENTION_MAX", "1"))
AG_SPATIAL_MAX = int(os.getenv("AG_SPATIAL_MAX", "5"))
AG_CONTACTING_MAX = int(os.getenv("AG_CONTACTING_MAX", "4"))
AG_ENFORCE_PERSON_SUBJECT = os.getenv("AG_ENFORCE_PERSON_SUBJECT", "1").strip().lower() in (
    "1", "true", "yes", "y"
)
...
_CANON_LABEL_RE = re.compile(r"^[a-z0-9]+(?:[a-z0-9_./-]*[a-z0-9])?$")
...
_RELPAIR_HDR_RE = re.compile(r"rel_pairs\[(\d+)\]\{.*?\}:", re.S | re.I)

_STRICT_OBJ_HDR_RE = re.compile(r"\A\s*obj\[(\d+)\]\{id,name,x1,y1,x2,y2\}:\s*\Z", re.I)
_STRICT_RELPAIR_HDR_RE = re.compile(
    r"\A\s*rel_pairs\[(\d+)\]\{subj,attention,spatial,contacting,obj\}:\s*\Z",
    re.I,
)

_LABEL_TOKEN = r"([a-z0-9]+(?:[a-z0-9_./-]*[a-z0-9])?)"
...
_STRICT_RELPAIR_LINE_RE = re.compile(
    r"\A\s*(\d+)\s*,\s*\[([a-z0-9_./,\s-]*)\]\s*,\s*\[([a-z0-9_./,\s-]*)\]\s*,\s*\[([a-z0-9_./,\s-]*)\]\s*,\s*(\d+)\s*\Z",
    re.I,
)
...
def _ag_atomic_pred(rel_type: str, label: str) -> str:
    lab = _category_key(label)
    return f"{rel_type}__{lab}" if lab else ""


def _parse_label_list_body_strict(raw: str, max_len: int, field_name: str) -> List[str]:
    raw = raw.strip()
    if not raw:
        return []
    items = [_canonical_label(x) for x in raw.split(",") if x.strip()]
    if any(not _is_valid_canonical_label(x) for x in items):
        raise ValueError(f"bad {field_name} labels: {raw!r}")
    if len(items) > max_len:
        raise ValueError(f"too many {field_name} labels: {len(items)} > {max_len}")
    if len(set(items)) != len(items):
        raise ValueError(f"duplicate {field_name} labels: {raw!r}")
    return items


def _parse_label_list_body_lenient(raw: str, max_len: int) -> Optional[List[str]]:
    try:
        return _parse_label_list_body_strict(raw, max_len=max_len, field_name="rel")
    except Exception:
        return None
...
def _build_gt_rel_index(gt_rels, gt_boxes, gt_names):
    ...
    for j, gr in enumerate(gt_rels):
        ...
        gp = _category_key(str(gr.get("predicate", "")))
...
# --------------------------
# Parsing: Action Genome TOON
# --------------------------
def _parse_scene_graph_ag_meta(text: str) -> Tuple[List[Obj], List[Rel], Dict[str, Any]]:
    pred_objs: List[Obj] = []
    atomic_rels: List[Rel] = []
    id_map: Dict[int, Obj] = {}

    m_obj = _OBJ_HDR_RE.search(text)
    m_rel = _RELPAIR_HDR_RE.search(text)
    ...
    obj_match = re.search(r"obj\[\d+\]\{.*?\}:(.*?)(?:\n\s*rel_pairs\[\d+\]|\Z)", text, re.S | re.I)
    if obj_match:
    ...
    rel_match = re.search(r"rel_pairs\[\d+\]\{.*?\}:(.*)$", text, re.S | re.I)
    if rel_match:
        for raw in rel_match.group(1).strip().splitlines():
            ...
            mr = _STRICT_RELPAIR_LINE_RE.fullmatch(line)
            if not mr:
            ...
            att = _parse_label_list_body_lenient(mr.group(2), AG_ATTENTION_MAX)
            spa = _parse_label_list_body_lenient(mr.group(3), AG_SPATIAL_MAX)
            con = _parse_label_list_body_lenient(mr.group(4), AG_CONTACTING_MAX)
            if att is None or spa is None or con is None:
                n_rel_lines_invalid_pred += 1
                continue
            if not (att or spa or con):
                n_rel_lines_invalid_pred += 1
                continue
            if AG_ENFORCE_PERSON_SUBJECT and _category_key(str(id_map[subj]["name"])) != "person":
                n_rel_lines_invalid_ref += 1
                continue

            n_pair_parsed += 1
            for rel_type, labels in (("attention", att), ("spatial", spa), ("contacting", con)):
                for lab in labels:
                    pred = _ag_atomic_pred(rel_type, lab)
                    if pred:
                        atomic_rels.append({"subject": subj, "predicate": pred, "object": obj})
    ...
    meta = {
        ...
        "n_pair_parsed": n_pair_parsed,
        "n_rel_parsed": len(atomic_rels),
    ...
    return pred_objs, atomic_rels, meta



def _parse_scene_graph_ag_strict(answer_text: str) -> Tuple[List[Obj], List[Rel], Dict[str, Any]]:
    ...
    for i in range(1, len(lines)):
        m_rel = _STRICT_RELPAIR_HDR_RE.fullmatch(lines[i])
        if m_rel:
            if rel_header_idx is not None:
                raise ValueError("multiple rel_pairs headers")
            rel_header_idx = i
            declared_m = int(m_rel.group(1))

    if rel_header_idx is None or declared_m is None:
        raise ValueError("missing rel_pairs header")

    obj_lines = lines[1:rel_header_idx]
    pair_lines = lines[rel_header_idx + 1:]

    if len(obj_lines) != declared_n:
        raise ValueError(f"declared_n={declared_n}, actual_obj_lines={len(obj_lines)}")
    if len(pair_lines) != declared_m:
        raise ValueError(f"declared_m={declared_m}, actual_pair_lines={len(pair_lines)}")
    ...
    name_by_id = {o["id"]: str(o["name"]) for o in objs}
    seen_pair_rows = set()
    seen_atomic = set()

    for ln in pair_lines:
        mr = _STRICT_RELPAIR_LINE_RE.fullmatch(ln)
        if not mr:
            raise ValueError(f"bad rel_pair line: {ln!r}")
        ...
        att = _parse_label_list_body_strict(mr.group(2), AG_ATTENTION_MAX, "attention")
        spa = _parse_label_list_body_strict(mr.group(3), AG_SPATIAL_MAX, "spatial")
        con = _parse_label_list_body_strict(mr.group(4), AG_CONTACTING_MAX, "contacting")
        if not (att or spa or con):
            raise ValueError(f"empty AG rel_pair labels: {ln!r}")
        if AG_ENFORCE_PERSON_SUBJECT and _category_key(name_by_id[subj]) != "person":
            raise ValueError(f"AG rel_pair subject must be person: {ln!r}")

        row_key = (subj, tuple(att), tuple(spa), tuple(con), obj)
        if row_key in seen_pair_rows:
            raise ValueError(f"duplicate rel_pair row: {ln!r}")
        seen_pair_rows.add(row_key)
        ...
        for rel_type, labels in (("attention", att), ("spatial", spa), ("contacting", con)):
            for lab in labels:
                ...
                if key in seen_atomic:
                    raise ValueError(f"duplicate atomic relation: {key!r}")
                seen_atomic.add(key)
                atomic_rels.append({"subject": subj, "predicate": pred, "object": obj})
    ...
    meta = {
        ...
        "n_pair_parsed": len(pair_lines),
        "n_rel_parsed": len(atomic_rels),
        "n_rel_lines_total": len(pair_lines),
        "n_rel_lines_invalid_ref": 0,
        "n_rel_lines_invalid_pred": 0,
    }
    return objs, atomic_rels, meta
...
def _parse_text_meta_cached(answer_text: str) -> Tuple[List[Obj], List[Rel], Dict[str, Any]]:
    return _parse_scene_graph_ag_meta(answer_text)
...
def _strict_valid_from_raw(raw_text: str) -> bool:
    try:
        ...
        objs, rels, _ = _parse_scene_graph_ag_strict(answer_body)
